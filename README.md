...
- 홈 화면
  - 사진 분석, 실시간 스캔, 신고 보조로 바로 이동
  - iPhone 상단 상태바와 하단 홈 인디케이터를 침범하지 않도록 safe-area 반영
  - 파스텔 톤 UI 적용
...
- 사진 분석
  ...
  - 결과 카드로 후보 타입, 근거, 위험도 표시

- 실시간 렌즈 반사 확인
  - iPhone 후면 카메라 스트림 사용
  - 프레임을 주기적으로 분석해 의심 후보 박스 표시
  - 위험도별 색상 표시
  - 기기에서 지원하는 경우 플래시 토글 시도
...
- 실제 AI 모델이 아니라 이미지 픽셀 기반 휴리스틱 탐지입니다.
- 조명, 거울, 금속 반사, 화면 가장자리에서 오탐이 발생할 수 있습니다.
- 플래시 제어는 iOS WebView와 기기 지원 여부에 따라 동작하지 않을 수 있습니다.
- 위치 자동 입력은 주소가 아니라 위도/경도 좌표로 입력됩니다.
- iOS QR 설치 배포는 TestFlight 또는 App Store 절차가 필요합니다.

## 다음 개선 후보

- 분석 결과 이미지 저장
- 신고 문안에 분석 결과 자동 포함
- 실시간 스캔 후보 깜빡임 완화를 위한 프레임 누적 필터
- 탐지 민감도 `낮음 / 보통 / 높음` 선택
- 권한 거부 시 설정 화면 이동 버튼
- 주소 변환 기반 위치 표시
